...
\setbeamertemplate{footline}{%
  ...
  \insertframenumber\,/\,\inserttotalframenumber\hspace{0.4cm}\vspace{0.15cm}%
}
...
% ---------- Steps at a glance ----------
\begin{frame}{The five steps --- what each one asks and answers}
  \small
  \begin{itemize}\setlength\itemsep{5pt}
    \item[\textcolor{UPAblue}{\textbf{1}}] \textbf{Hard margin --- can a perfect line exist?}
      Demand a straight boundary that classifies \emph{every} patient correctly with a margin.
      This is a convex feasibility problem: either such a line exists, or it provably does not.
    \item[\textcolor{BPAorange}{\textbf{2}}] \textbf{Prove it is impossible.}
      Three independent checks agree the classes are not linearly separable:
    ...
    \item[\textcolor{UPAblue}{\textbf{3}}] \textbf{Relax to the soft margin.}
      Allow mistakes but charge for them through slack variables and a penalty $C$.
      Always solvable; \emph{(3a)} tune $C$ by grid search, \emph{(3b)} read it as a Pareto
      trade-off, \emph{(3c)} see the fitted line, \emph{(3d)} solve the primal directly by SGD.
    \item[\textcolor{BPAorange}{\textbf{4}}] \textbf{Compare against a curved boundary.}
      Swap the inner product for an RBF kernel so the boundary can bend, then check
      whether the extra flexibility actually helps or merely overfits.
  \end{itemize}
  \take{Steps 1--2 establish \emph{what is impossible}; steps 3--4 find \emph{the best thing that is possible}.}
\end{frame}
...
\begin{frame}{Descriptive statistics --- the subtype contrast}
  ...
  \begin{columns}[T]
    \begin{column}{0.58\textwidth}
      ...
      \end{tabular}
      \\[0.4em]
      {\tiny Bold: $p<0.05$ (Welch $t$-test), i.e.\ separates the subtypes (7 of 10).
...
% ---------- SGM on the primal ----------
\begin{frame}{Step 3d --- reaching the same solution with SGD}
  \lecref{The Stochastic Gradient Method; Strong Convexity \& Descent (W10)}
  \small
  The soft-margin objective is a \textbf{finite sum} over the $m$ patients. This means we
  can optimize it one randomly selected patient at a time, without solving the dual QP:
  $w_{k+1}=w_k-\eta_k g_k$ with
  $g_k=\lambda w_k$ if $t_k w_k^\top x_k\ge 1$, else $g_k=\lambda w_k-t_k x_k$
  (step $\eta_k=1/(\lambda k)$; at the hinge, we use a \emph{subgradient} together with
  the regularization term). It comes within $5\%$ of the optimum after about 14 epochs and
  within $1\%$ after about 67. Its final direction closely matches the dual solution
  (cosine $0.99$; AUROC $0.958$ vs $0.955$).
  \vspace{-0.4em}
  \begin{center}
    \includegraphics[width=0.92\textwidth,height=0.48\textheight,keepaspectratio]{output/figures/sgm_convergence_slide.pdf}
  \end{center}
  \vspace{-0.5em}
  \take{SGD gets very close to the dual solution. The remaining fluctuations reflect stochastic optimization, not a new test of separability.}
\end{frame}
  ...
  \begin{columns}[T]
    \begin{column}{0.40\textwidth}
      ...
      \begin{itemize}\setlength\itemsep{2pt}
        ...
        \item It bends \emph{around} points, yet the two hulls still overlap.
        \item Curvature relabels a few borderline points but does not separate the classes.
      \end{itemize}
...
\begin{frame}{Does the curved boundary actually help?}
  \begin{columns}[T]
    ...
    \begin{column}{0.45\textwidth}
      ...
      \end{itemize}
      \vspace{0.4em}
      \alert{In practical terms, the two models discriminate equally well.}
    \end{column}
  \end{columns}
  \take{Across repeated splits, the RBF model shows no reliable advantage over the simpler linear model.}
\end{frame}
