...
% ---------- SGM on the primal ----------
\begin{frame}{Step 3d --- solving the primal directly with SGD}
  \lecref{The Stochastic Gradient Method; Strong Convexity \& Descent (W10)}
  \small
  The soft-margin primal is a \textbf{finite sum} over the $m$ patients, so it can be
  solved one random point at a time (no dual, no QP):
  $w_{k+1}=w_k-\eta_k g_k$ with
  $g_k=\lambda w_k$ if $t_k w_k^\top x_k\ge 1$, else $g_k=\lambda w_k-t_k x_k$
  (step $\eta_k=1/(\lambda k)$; the hinge kink makes $g_k$ a \emph{subgradient}, as in
  the lecture, plus the regularizer term). Reaches within $5\%$ of the optimum in $\approx14$ epochs, $1\%$
  in $\approx67$, and matches the dual solver (cosine $0.99$, AUROC $0.958$ vs $0.955$).
  \vspace{-0.4em}
  \begin{center}
    \includegraphics[width=0.92\textwidth,height=0.48\textheight,keepaspectratio]{output/figures/sgm_convergence_slide.pdf}
  \end{center}
  \vspace{-0.5em}
  \take{SGD reaches the same hyperplane as the dual --- but the noisy floor is a tell: the data are not separable ($B>0$).}
\end{frame}
