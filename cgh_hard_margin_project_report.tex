...
Primary aldosteronism (PA) is an important cause of secondary hypertension. The clinically relevant subtype decision is whether PA is unilateral or bilateral. Unilateral PA can often be treated surgically, whereas bilateral PA is usually managed medically \cite{rossi2019}. The aim of this project is to formulate this binary classification problem as a maximum-margin optimization problem, test whether a linear hard-margin support vector machine (SVM) is feasible on the available project data, and, if it is not, identify a validated soft-margin alternative.
...
$20260710$. The subtype label is drawn first, after which the ten continuous
...
population. Consequently, both separability and predictive performance partly
reflect the prespecified simulation assumptions and must be interpreted only
as demonstrations of the optimization workflow. A binary Sex variable is
also generated for descriptive context but is excluded a priori from the SVM
feature vector so that the analysis remains restricted to the ten continuous
...
The final full-data grid search also selected $C=0.01$, with mean inner-fold
...
outer folds, the primary estimate is mean fold-wise AUROC $0.95$; pooling all
...
\cite{lec-strongconvex}, while the intercept remains unpenalized. The baseline
...
lecture \cite{lec-momentum} are also evaluated:
\begin{equation}
\begin{aligned}
\text{Heavy Ball:}\quad
...
lowest final averaged-iterate objective is reported. This tuning is only an
...
\scriptsize
\resizebox{\textwidth}{!}{\input{output/tables/primal_optimizer_comparison_table.tex}}
\end{table}
...
The comparison does not show a meaningful momentum advantage. SSG reaches the
...
head-to-head estimate, however, because different outer folds selected
different $(C,\gamma)$ pairs and their raw decision scores are not calibrated
to a common scale. Pooling them can change ranks across folds even though each
fold-specific ROC curve is unchanged. For that reason, the pooled gap and its
naive DeLong or patient-bootstrap significance tests are not used to choose the
model. No formal inference is attached to this scale-sensitive pooled
calculation.
...
not twenty independent experiments. They show that the direction of the tiny
...
$C=0.01$, and nested cross-validation yielded mean outer-fold AUROC $0.95$.
The optimized
...
effect of the RBF inverse-bandwidth parameter $\gamma$ (panel D), and a symbol key with the final
...
\begin{figure}[p]
    ...
    $C$. Panel D: smooth versus memorizing RBF surfaces as $\gamma$ grows.
...
The supplied numerical artifacts were audited against the project scripts and
...
\begin{itemize}
    ...
    reported numerical results. The only material run-to-run comparison noted
    in the original model pipeline is the RBF
    ...
    to six decimal places within the supplied project pipeline.
...
figures, and numerical audit files.
