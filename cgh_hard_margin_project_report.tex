...
\input{output/tables/synthetic_target_audit_table.tex}
\end{table}
...
\end{aligned}
\end{equation}
The displayed certificate weights are rounded, so these identities must be
checked with the stored full-precision values. The final residual is reported
...
\end{aligned}
\end{equation}
The displayed arithmetic uses rounded components; the audit and duality gap
below use the stored full-precision coefficients and slacks.
...
\scriptsize
\resizebox{\textwidth}{!}{\input{output/tables/primal_optimizer_robustness_table.tex}}
\end{table}
...
\scriptsize
\resizebox{\textwidth}{!}{\input{output/tables/boundary_sensitivity_table.tex}}
\end{table}
...
similar validation with fewer tuning degrees of freedom and a directly
...
\label{tab:baselines}
\input{output/tables/baseline_comparison_table.tex}
\end{table}
