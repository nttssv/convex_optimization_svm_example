...
\begin{equation}
    ...
    =\begin{bmatrix}I_p&0\\0&0\end{bmatrix}.
\end{equation}
When the primal is feasible, its Karush--Kuhn--Tucker (KKT) conditions are necessary and sufficient for global optimality \cite{boyd2004}:
\begin{equation}
\begin{aligned}
...
\section{Linear Hard-Margin Feasibility and Training Code}
...
\lstinputlisting[language=Python]{scripts/linear_hard_margin_svm.py}

\section{Linear Soft-Margin Grid Search and Audit Code}

The implementation below performs leakage-safe nested cross-validation,
selects $C$ by AUROC, fits the final linear soft-margin model, checks its
primal and dual objectives, and generates the CSV tables and figures used in
Section~\ref{sec:soft-margin}.

\lstinputlisting[language=Python]{scripts/linear_soft_margin_svm.py}

\section{RBF Kernel Grid Search and Overfitting Audit Code}

The implementation below jointly tunes $C$ and $\gamma$ inside nested
cross-validation, evaluates train--validation gaps, compares RBF and linear
outer-fold performance, and generates the Chapter~\ref{sec:kernel-rbf}
artifacts.

\lstinputlisting[language=Python]{scripts/kernel_rbf_svm.py}
